...
import { createMessage, listMessages } from '../src/server/messages/store.js';
...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    const messages = await listMessages(config, articleSlug);
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    assertMessageRateLimit(fingerprint);

    const message = await createMessage(config, input);
